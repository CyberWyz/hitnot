...
$students_query = mysqli_query($con, "SELECT * FROM users ORDER BY Username ASC");
...
if (isset($_POST['submit'])) {
    ...
    $check_rfid = mysqli_query($con, "SELECT * FROM assets WHERE rfid_uid = '$rfid_uid'");
    if (mysqli_num_rows($check_rfid) > 0) {
        $error = "RFID tag already exists in the system.";
    } 
    // Check if serial number already exists
    else if (!empty($serial_number)) {
        $check_serial = mysqli_query($con, "SELECT * FROM assets WHERE serial_number = '$serial_number'");
    ...
    if (empty($error)) {
        $insert_query = mysqli_query($con, "INSERT INTO assets (item_description, item_model, serial_number, rfid_uid, reg_number, AssetStatus) 
                                          VALUES ('$item_description', '$item_model', '$serial_number', '$rfid_uid', '$reg_number', '$asset_status')");
        
        if ($insert_query) {
            // Log the action
            mysqli_query($con, "INSERT INTO admin_logs (admin_id, action, details, ip_address) 
                              VALUES ($admin_id, 'add_asset', 'Added new asset: $item_description (S/N: $serial_number)', '{$_SERVER['REMOTE_ADDR']}')");
            ...
            $error = "Failed to add asset: " . mysqli_error($con);
        }
    }
}
?>
        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-header">
                    <h2><i class="fas fa-plus-circle"></i> Asset Information</h2>
                </div>
                <div class="card-body">
                    <?php if (!empty($error)): ?>
                    ...
                    <form action="" method="post">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="item_description" name="item_description" value="<?php echo isset($_POST['item_description']) ? htmlspecialchars($_POST['item_description']) : ''; ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="item_model">Model/Make</label>
                            <input type="text" class="form-control" id="item_model" name="item_model" value="<?php echo isset($_POST['item_model']) ? htmlspecialchars($_POST['item_model']) : ''; ?>">
                        </div>
                        
                        <div class="form-group">
                            <label for="serial_number">Serial Number</label>
                            <input type="text" class="form-control" id="serial_number" name="serial_number" value="<?php echo isset($_POST['serial_number']) ? htmlspecialchars($_POST['serial_number']) : ''; ?>">
                        </div>
                        
                        <div class="form-group">
                            <label for="rfid_uid">RFID Tag UID *</label>
                            <input type="text" class="form-control" id="rfid_uid" name="rfid_uid" value="<?php echo isset($_POST['rfid_uid']) ? htmlspecialchars($_POST['rfid_uid']) : ''; ?>" required>
                            ...
                            <div class="rfid-section">
                                ...
                                <div id="rfid-status" class="rfid-status status-waiting">
                                    Click the button above to scan for RFID tags
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="reg_number" name="reg_number">
                                ...
                                <?php while ($student = mysqli_fetch_assoc($students_query)): ?>
                                    <option value="<?php echo $student['Reg_Number']; ?>" <?php echo (isset($_POST['reg_number']) && $_POST['reg_number'] == $student['Reg_Number']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="asset_status" name="asset_status" required>
                                <option value="Active" <?php echo (isset($_POST['asset_status']) && $_POST['asset_status'] == 'Active') ? 'selected' : ''; ?>>Active</option>
                                <option value="Missing" <?php echo (isset($_POST['asset_status']) && $_POST['asset_status'] == 'Missing') ? 'selected' : ''; ?>>Missing</option>
                                <option value="Blacklisted" <?php echo (isset($_POST['asset_status']) && $_POST['asset_status'] == 'Blacklisted') ? 'selected' : ''; ?>>Blacklisted</option>
                            </select>
                        </div>
